 ...
 *
 * `weeklyProjByWeek` is keyed by week number (`{1: {teamId: score}, ...}`)
 * rather than a single flat map, since each graded week needs its own
 * forward projection — reusing one week's numbers for another would leak
 * a stale forecast into weeks it was never made for.
 */
export function modelRecord({ teams, matchups, rosterProj, baseline, weeklyProjByWeek = {}, throughWeek }) {
  ...
  weeks.forEach(w => {
    ...
    const games = gradeWeek({ teams, matchups, rosterProj, baseline, weeklyProj: weeklyProjByWeek[w] || {}, week: w })
    let favW = 0, favL = 0
    games.forEach(g => {
      rec.games++
