...
export function simulateLeverage({ rows, schedule = [], randomWeeks = 0, trackWeek = null, sims = 8000 }) {
  ...
  const tally = (accs, w, p) => {
    ...
    if (n >= PLAYOFF_SPOTS) {
      ...
      pl.slice(0, BYE_SPOTS).forEach(i => { gotBye[i] = true; inSemis[i] = true })
      const q1 = winnerOf(pl[2], pl[5])
      const q2 = winnerOf(pl[3], pl[4])
      inSemis[q1] = true; inSemis[q2] = true
      const worse = pl.indexOf(q1) > pl.indexOf(q2) ? q1 : q2
      const better = worse === q1 ? q2 : q1
      const f1 = winnerOf(pl[0], worse)
      const f2 = winnerOf(pl[1], better)
