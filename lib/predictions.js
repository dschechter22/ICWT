...
// ICWT runs an 8-team single-elimination bracket (1v8/4v5 feed one semi,
// 3v6/2v7 feed the other) out of 10 teams -- 8 is already a power of two,
// so no seed gets a bye.
export const PLAYOFF_SPOTS = 8
export const BYE_SPOTS = 0
...
export function simulateFutures({ rows, schedule = [], randomWeeks = 0, sims = 10000 }) {
  ...
  for (let s = 0; s < sims; s++) {
    ...
    // 1v8 and 4v5 feed one semifinal, 3v6 and 2v7 feed the other.
    const topA = winnerOf(p[0], p[7])
    const topB = winnerOf(p[3], p[4])
    const botA = winnerOf(p[2], p[5])
    const botB = winnerOf(p[1], p[6])
    counts[topA].semis++; counts[topB].semis++; counts[botA].semis++; counts[botB].semis++
